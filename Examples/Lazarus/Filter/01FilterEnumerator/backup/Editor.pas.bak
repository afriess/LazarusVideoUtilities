...
uses
  ...
  Controls, Forms, Dialogs, StdCtrls, DXSUtil, DirectShow9, ComCtrls, DSPack,
  Menus, Interfaces, BaseFilterEditor;
...
type
  TFormEditor = class(TForm)
    ...
    procedure PopupMenuePopup(Sender: TObject);
...
procedure TFormEditor.PopupMenuePopup(Sender: TObject);
begin
  PopupMenue.Items.Items[0].Enabled := false;
  PopupMenue.Items.Items[1].Items[0].Enabled := false;
  PopupMenue.Items.Items[1].Items[1].Enabled := false;
  PopupMenue.Items.Items[1].Items[2].Enabled := false;
  PopupMenue.Items.Items[2].Items[0].Enabled := false;
  PopupMenue.Items.Items[2].Items[1].Enabled := false;
  if lbFilters.ItemIndex = -1 then exit;
  if HaveFilterPropertyPage(Filter as IBaseFilter, ppDefault)       then PopupMenue.Items.Items[0].Enabled := true;
  if HaveFilterPropertyPage(Filter as IBaseFilter, ppVFWCapFormat)  then PopupMenue.Items.Items[1].Items[0].Enabled := true;
  if HaveFilterPropertyPage(Filter as IBaseFilter, ppVFWCapSource)  then PopupMenue.Items.Items[1].Items[1].Enabled := true;
  if HaveFilterPropertyPage(Filter as IBaseFilter, ppVFWCapDisplay) then PopupMenue.Items.Items[1].Items[2].Enabled := true;
  if HaveFilterPropertyPage(Filter as IBaseFilter, ppVFWCompConfig) then PopupMenue.Items.Items[2].Items[0].Enabled := true;
  if HaveFilterPropertyPage(Filter as IBaseFilter, ppVFWCompAbout)  then PopupMenue.Items.Items[2].Items[1].Enabled := true;
